...
export default function AdminPanel() {
  ...
  async function openSalaryModal(u) {
    setSalaryModal(u); setSalaryAmount(""); setSalaryNote("");
    setModalPending(0); setModalLoading(true);
    try {
  ...
  async function handlePaySalary() {
    ...
    try {
      ...
      setSalaryModal(null); setSalaryAmount(""); setSalaryNote("");
  ...
  async function handleSendNote(e) {
    ...
    setSendingNote(true);
    try {
      await updateDoc(doc(db, "users", noteModal.id), {
        pendingNote: noteText.trim(),
        noteRead: false,
      });
  ...
  function openUpdateModal(u) {
    setUpdateModal(u);
    setUpdateForm({ name: u.name, phone: u.phone || "", cnic: u.cnic || "", role: u.role || "sales" });
  }
                        ...
                        <div className="dot-menu-wrap" onClick={(e) => e.stopPropagation()}>
                          ...
                          {menuOpen === u.id && (
                            <div className="dot-dropdown">
                              ...
                              <button className="delete-opt" onClick={(e) => { e.stopPropagation(); setMenuOpen(null); handleDelete(u); }}>🗑️ Delete</button>
                            </div>
                          )}
                        </div>
                      </div>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSendNote}>
              <div className="form-group" style={{ marginTop: 16 }}>
                ...
                <textarea
                  placeholder="Write a note for this employee..."
                  value={noteText}
              ...
              <p style={{ fontSize: 12, color: "var(--text-light)", marginTop: 8 }}>
                📌 This note will appear as a popup when the employee next opens their dashboard (shown once only).
              </p>
        ...
        <div className="modal-overlay" onClick={() => setUpdateModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleUpdate}>
              ...
              <div className="form-group" style={{ marginTop: 12 }}>
                <label>Phone</label>
                <input type="tel" value={updateForm.phone} onChange={(e) => setUpdateForm({ ...updateForm, phone: e.target.value })} />
              </div>
              <div className="form-group" style={{ marginTop: 12 }}>
                <label>CNIC</label>
                <input type="text" value={updateForm.cnic} onChange={(e) => setUpdateForm({ ...updateForm, cnic: e.target.value })} />
              </div>
              <div className="form-group" style={{ marginTop: 12 }}>
                <label>Role</label>
